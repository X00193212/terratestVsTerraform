...
func TestEndToEndDeploymentScenario(t *testing.T) {
	...
	test_structure.RunTestStage(t, "validate", func() {
		...
		attemptsCount := 0
		for vmLinux1PublicIPAddress == "" && attemptsCount < 60 {
		...
		port := ":22"
		vmLinux1PublicIPAddresswithport := strings.Replace(vmLinux1PublicIPAddress, `"`, "", -1) + port
		fmt.Println("Host and Port:", vmLinux1PublicIPAddresswithport)

		//sshConnection, err := ssh.Dial("tcp", fmt.Sprintf("%s:22", vmLinux1PublicIPAddress), sshConfig)
		sshConnection, err := ssh.Dial("tcp", vmLinux1PublicIPAddresswithport, sshConfig)
		if err != nil {
		...
		if err != nil {
			t.Fatalf("Cannot ping vm-linux-2 from vm-linux-1: %v", err)
		}
	})
